            ...
            do
            {
                ...
                switch (opcion)
                {
                    ...
                    case 5: // Se utiliza el gettype
                        if (miCola.GetType() == typeof(Queue))
                            Console.WriteLine("Si comprueba");
                        break;
                    case 6://Se utiliza el tostring
                        Console.WriteLine("El objeto seleccionado es: " + miCola.ToString());
                        break;
                    case 7: // Se utiliza el toarray
                        Console.WriteLine("El elemento en el arreglo es: " + miCola.ToArray().ElementAt(0));
                        break;

                    case 8: // se utiliza el GetEnumerator
                        IEnumerator ej = miCola.GetEnumerator();
                        while (ej.MoveNext())
                        {
                ...
                Console.WriteLine("La pila tiene {0} elementos", miCola.Count);
